...
func TestBuildCommand(t *testing.T) {
	...
	if _, err := BuildRequestDto(query1); err == nil {

		t.Errorf("fail to build %s query", query1)
	}

	if _, err := BuildRequestDto(query2); err == nil {

		t.Errorf("fail to build %s query", query2)
	}

	if _, err := BuildRequestDto(query3); err == nil {

		t.Errorf("fail to build %s query", query3)
	}

	if _, err := BuildRequestDto(query4); err == nil {

		t.Errorf("fail to build %s query", query4)
	}

	if _, err := BuildRequestDto(query5); err == nil {

		t.Errorf("fail to build %s query", query5)
	}

	cmd6, err := BuildRequestDto(query6)
	...
	cmd7, err := BuildRequestDto(query7)
